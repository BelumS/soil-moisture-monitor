...
const struct s_address addresses[] =
{
    ...
    {S_CALLSIGN, S_CALLSIGN_ID},  // Source callsign (-9 = car, -13 = weather)
    #ifdef DIGI_PATH1
...
void aprs_send()
{
  ...
  ax25_send_byte('/');                // Status Report w/ timestamp, no APRS messaging. $ = NMEA raw data

  /* Information gathered from the Sensors */
  ax25_send_string("Mois=");    
  snprintf(temp, 6, "%02d%%", sensors_dfr193_soil());  //Soil moisture content
...
void aprs_send()
{
  ...
  ax25_send_string("Mois=");    
  snprintf(temp, 6, "%02d%%", sensors_dfr193_soil()); 
  ax25_send_string(temp);
  ax25_send_byte(' ');
...
/*  allocate some memory for our analog and digital telemetry values, these
...
extern const uint8_t telem_param[] PROGMEM = ":KR3ESE-13 :PARAM.Battery,ITemp,Etemp,,,Chut,,,,,,,\0";
extern const uint8_t telem_units[] PROGMEM = ":KR3ESE-13 :UNIT.V,degC,degC,,,Open,,,,,,,\0";
extern const uint8_t telem_coeff[] PROGMEM = ":KR3ESE-13 :EQNS.0,0.0129,0,0,1,-128,0,1,-128,0,0,0,0,0,0\0";
extern const uint8_t telem_bits[]  PROGMEM = ":KR3ESE-13 :BITS.11111111, Soil Monitoring StationB\0";
extern const uint8_t telem_data[TELEM_DATA_PACKETS] PROGMEM = { 
                                                           &telem_param[0],  \
                                                           &telem_units[0],  \
                                                           &telem_coeff[0],  \
                                                           &telem_bits[0] };
